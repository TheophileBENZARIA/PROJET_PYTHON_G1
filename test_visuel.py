"""
Test visuel spécial pour voir clairement les différences entre Daft et Braindead
Version optimisée pour l'observation des stratégies
"""
...
def test_visuel_simple():
    ...
    try:
        ...
        if hasattr(result, 'winner'):
            ...
            turns = result.turns
            print(f"🏆 Vainqueur: {winner if winner else 'Match nul'}")
            print(f"⏱️ Tours ecoules: {turns}")
        else:
    ...
    for i in range(3):
        ...
        try:
            ...
            if hasattr(result, 'winner'):
                ...
                turns = result.turns
                if winner == "Rouge":
                    print(f"🏆 BRAINDEAD gagne en {turns} tours")
                elif winner == "Bleue":
                    print(f"🏆 DAFT gagne en {turns} tours")
                else:
                    print(f"🤝 Match nul en {turns} tours")
            else:
                print("📊 Bataille terminee")
...
def test_visuel_detaille():
    ...
    print("=" * 50)
    print("🛡️ CAPTAIN BRAINDEAD (Defensif):")
    ...
    print("=" * 50)
    print("1. 🔴 Les unites ROUGES (Braindead) restent immobiles")
    print("2. 🔵 Les unites BLEUES (Daft) avancent vers la gauche")
    print("3. ⚔️ Quand Daft arrive a portee, il attaque")
    print("4. 💥 Vous verrez les degats infliges")
    print("5. 🏆 Le vainqueur sera determine")
...
if __name__ == "__main__":
    try:
        ...
        print(f"[OK] Test visuel termine!")
