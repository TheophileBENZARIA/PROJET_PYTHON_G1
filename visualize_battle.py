"""
Script de visualisation des batailles entre Daft et Braindead
Montre le déroulement visuel des batailles étape par étape
"""
...
def create_visual_battle():
    ...
    # Ajout des unités
    for i, pos in enumerate(positions_army1):
        unit_type = [UnitType.KNIGHT, UnitType.ARCHER, UnitType.SPEARMAN, UnitType.SWORDSMAN][i % 4]
        army1.add_unit(Unit(unit_type, pos))
    
    for i, pos in enumerate(positions_army2):
        unit_type = [UnitType.KNIGHT, UnitType.ARCHER, UnitType.SPEARMAN, UnitType.SWORDSMAN][i % 4]
        army2.add_unit(Unit(unit_type, pos))
    ...
    winner_name, total_turns, actions_count = engine.run_battle(braindead, daft, army1, army2)
...
def show_multiple_battles():
    ...
    for i in range(5):
        ...
        winner_name, total_turns, _ = engine.run_battle(braindead, daft, army1, army2)
        
        if winner_name == "Armee Rouge":
            braindead_wins += 1
            print(f"🏆 Vainqueur: {braindead.get_name()} ({total_turns} tours)")
        elif winner_name == "Armee Bleue":
            daft_wins += 1
            print(f"🏆 Vainqueur: {daft.get_name()} ({total_turns} tours)")
        else:
            print(f"🤝 Match nul ({total_turns} tours)")
    
    print(f"\nRESULTATS FINAUX:")
    print("=" * 30)
    print(f"🛡️ {braindead.get_name()}: {braindead_wins} victoires")
    print(f"⚔️ {daft.get_name()}: {daft_wins} victoires")
    print(f"🤝 Matchs nuls: {5 - braindead_wins - daft_wins}")
    
    if braindead_wins > daft_wins:
        print(f"\n🏆 {braindead.get_name()} est plus efficace!")
    elif daft_wins > braindead_wins:
        print(f"\n🏆 {daft.get_name()} est plus efficace!")
    else:
        print(f"\n🤝 Les deux generaux sont equilibres!")
